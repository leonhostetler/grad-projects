...
using the method of ghost points. We end up with a set of linear equations,
...
def fdm1(left, right, h):
    ...
    bmat = 0.5*h*np.array((f(0.0)))        # sigma + h*f(0)/2
    zmat = np.array((0.0))                 # beta
    F = np.hstack((bmat,F,zmat))           # Add the boundary points
