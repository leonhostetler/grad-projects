...
using a FD approximation derived using the method of undetermined coefficients in
order to maintain second-order accuracy. This gives us a set of linear equations,
...
def fdm1(left, right, h):
    ...
    A[0,0],A[0,1],A[0,2] = 1.5*h,-2*h,0.5*h # Left boundary points
...
# Get the slopes
logE2, logE1 = np.log10(np.abs(inf_list[2])), np.log10(np.abs(inf_list[0]))
logh2, logh1 = np.log10(np.abs(hlist[2])), np.log10(np.abs(hlist[0]))
p = (logE2-logE1)/(logh2-logh1)
print("\nFor ||E||_inf: p = ", p)

logE2, logE1 = np.log10(np.abs(l1_list[2])), np.log10(np.abs(l1_list[0]))
p = (logE2-logE1)/(logh2-logh1)
print("\nFor ||E||_1: p = ", p)

logE2, logE1 = np.log10(np.abs(l2_list[2])), np.log10(np.abs(l2_list[0]))
p = (logE2-logE1)/(logh2-logh1)
print("\nFor ||E||_2: p = ", p)
